...
const Autocomplete = props => {
    const autocompleteItems = props.recommendations.map((item, index) => {
        return (
            <Link to={`/player_profile/${item.id}`} key={index}>
                <div
                    className={`autocomplete-item col-12`}
                    key={index}
                >
                    ...
                    <div className="col-2">
                        {" "}
                        <img
                            className="autocomplete-pic"
                            src={
                                !images[`player_pic/${item.tag}.png`] ? (
                                    ProfilePlaceholder
                                ) : (
                                    images[`player_pic/${item.tag}.png`]
                                )
    ...
    return <div className="autocomplete">{autocompleteItems}</div>;
};
